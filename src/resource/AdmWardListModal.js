...
const AdmWardListModal = ({ isOpen, closeWardListModal, admissionRequestDto, onSelection }) => {
  ...
  console.log(admissionRequestDto.department);
  ...
  const getWardsByDepartment = async (department) => {
    try {
      const response = await axios.get(`${url}/wardsDept?department=${admissionRequestDto.department}`);
  ...
  return (
    <div className="adm-admissionRequestModal" style={{ display: isOpen ? "block" : 'none' }}>
      <div className="wardListBox">
        <div id="LboxHeader"
        ><br />
          <h3 id="boxHeader"><img id="boxIcon" style={{ marginTop: "-5px", marginLeft: "-10px" }} src="./img/notice.png" /> &nbsp;&nbsp;{admissionRequestDto.departmentName}병동 조회
          </h3>
        </div>
        <br /><br />
          ...
          <div id="wardList">
            <select size="10" className="wardOption" value={selectedWard} onChange={(e) => setSelectedWard(e.target.value)}>
              ...
              {wards.map((ward, index) => (
                <option value={ward} onClick={(e) => getRoomsByWard(e.target.value)} style={{ marginBottom: "10px" }}>{ward} 병동</option>
              ))}
            </select>
          </div>
        </div>
